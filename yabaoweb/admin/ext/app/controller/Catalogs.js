Ext.define('M.controller.Catalogs', {
    ...
    deletethirdClick: function(o, item, rowIndex, colIndex, e){
        ...
        }else if(store.getAt(rowIndex).get('id') == 23){
            alert(store.getAt(rowIndex).get('name') +"不能删除");
        }else{
            Ext.Msg.show({
                 title:'温馨提示',
    ...
    deleteClick: function(o, item, rowIndex, colIndex, e){
        ...
        if(store.getAt(rowIndex).get('id') == null || store.getAt(rowIndex).get('id') == ''){
            store.removeAt(rowIndex);
        }else if(store.getAt(rowIndex).get('id') == 7){
            alert(store.getAt(rowIndex).get('name') +"不能删除");
        }else{
            Ext.Msg.show({
                 title:'温馨提示',
    ...
    informationdownClick: function(o, item, rowIndex, colIndex, e){
    	...
    	if(store.getAt(rowIndex).get('id') == null || store.getAt(rowIndex).get('id') == ''){
			...
			downstore.getProxy().url = "../admin/catalog.action?type=allsecond_"+ store.getAt(rowIndex).get('id');
			downstore.load();
			var comboxid = Ext.getCmp('secondcatalogid');
			comboxid.store.proxy.url = '../admin/catalog.action?type=allsecond_'+ store.proxy.url.split("_")[1];
			comboxid.store.reload();
    	}
    },
