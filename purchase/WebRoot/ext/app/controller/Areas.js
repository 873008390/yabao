Ext.define('M.controller.Areas', {
    extend: 'Ext.app.Controller',

    stores: [
             'Citys',
             'Provinces',
             'Towns'
         ],
    
    models: [
             'City',
             'Province',
             'Town'
            ],
         
    views: [
            'area.ProvinceList',
            'area.CityList',
            'area.TownList'
        ],
       
    init: function() {
    	this.control({
    		'areaprovincelist button[id=deleteprovince]':{
           	 	click: this.deleteprovinceClick
            },
            'areaprovincelist button[id=information]':{
           	 	click: this.informationprovinceClick
            },
            'areaprovincelist button[id=addprovince]':{
              	click: this.addprovinceClick
               },
            'areaprovincelist button[id=saveprovince]': {
                click: this.updateprovinceClick
            },
            'areacitylist button[id=deletecity]':{
           	 	click: this.deletecityClick
            },
            'areacitylist button[id=back]':{
           	 	click: this.backClick
            },
            'areacitylist button[id=addcity]':{
              	click: this.addcityClick
               },
            'areacitylist button[id=savecity]': {
                click: this.updatecityClick
            },
            'areatownlist actioncolumn[id=deletetown]':{
           	 	click: this.deletetownClick
            },
            'areatownlist button[id=addtown]':{
              	click: this.addtownClick
               },
            'areatownlist button[id=savetown]': {
                click: this.updatetownClick
            },
            'areaprovincelist button[id=searchprovince]': {
                click: this.searchprovinceClick
            },
            'areacitylist button[id=searchcity]': {
                click: this.searchcityClick
            },
            'areatownlist button[id=searchtown]': {
                click: this.searchtownClick
            }
        });
    },
    
    backClick:function(o){
    	history.back();
    },
	 
    searchprovinceClick: function(o){
		//alert(Ext.getCmp("keyword").getValue());
		var grid = o.up('areaprovincelist');
		var store = grid.getStore();
		store.getProxy().url = "province/list.action?idtype=search&typeid="+ Ext.getCmp("keywordprovince").getValue();
		store.load();
	 },
	 
    searchcityClick: function(o){
		//alert(Ext.getCmp("keyword").getValue());
		var grid = o.up('areacitylist');
		var store = grid.getStore();
		store.getProxy().url = "city/list.action?idtype=search&typeid="+ Ext.getCmp("keywordcity").getValue();
		store.load();
	 },
	 
    searchtownClick: function(o){
		//alert(Ext.getCmp("keyword").getValue());
		var grid = o.up('areatownlist');
		var store = grid.getStore();
		store.getProxy().url = "town/list.action?idtype=search&typeid="+ Ext.getCmp("keywordtown").getValue();
		store.load();
	 },
    
    informationprovinceClick: function(o){
    	var grid = o.up('areaprovincelist');
    	var store = grid.getStore();
    	var model = grid.getSelectionModel();
    	if(model.hasSelection()){
	    	var record = model.getLastSelected();
	    	if(record.get('id') == null || record.get('id') == ''){
	    		alert("请先保存");
	    	}else{
	    		self.location = "areacitylist.html?idtype=province&typeid="+ record.get('id') +"&name="+ escape(record.get('name'));
	    	}
    	}else{
    		alert("请选择省份");
    	}
    },
    
    deleteprovinceClick: function(o){
    	var grid = o.up('areaprovincelist');
    	var store = grid.getStore();
    	var model = grid.getSelectionModel();
    	if(model.hasSelection()){
	    	var record = model.getLastSelected();
    		var rowIndex = store.indexOf(record);
	    	if(record.get('id') == null || record.get('id') == ''){
	    		store.removeAt(rowIndex);
	    	}else{
	    		var message = confirm("确定要删除："+ record.get('name') +" ？");
	    		if(message == true){
	    			 Ext.getBody().mask("数据处理中，请稍等");
   		             var result = "";
   		             Ext.Ajax.request({
   		    				url: 'province/delete.action?provinceid='+ record.get('id'),
   		    				method: 'GET',
   		    				timeout: 4000,
   		    				success: function(response,opts){
   		    					var result = Ext.JSON.decode(response.responseText);
   		    					alert("温馨提示:"+result.result);
   		    					if(result.result == "删除成功"){
   		    						store.removeAt(rowIndex);
   		    					}
   	    			    	    Ext.getBody().unmask();
   		    				},
   		    				failure: function(response,opts){
   		    					var result = Ext.JSON.decode(response.responseText);
   		    					alert("温馨提示:"+result.result);
   	    			    	    Ext.getBody().unmask();
   		    				}
   		    		 });
	    		}
	    	}
    	}else{
    		alert("请选择省份");
    	}
    },
	 
	addprovinceClick: function(o){
		var rec = new M.model.City();
		//var grid = o.ownerCt.ownerCt;
		var grid = o.up('areaprovincelist');
		var edit = grid.getPlugin('celledit');
		edit.cancelEdit();
		grid.getStore().insert(0, rec);
		edit.startEditByPosition({
			row:0,
			column:1
		});
	 },

    updateprovinceClick: function(o) {
    	var grid = o.up('areaprovincelist');
    	cellEdit.completeEdit();
		var store = grid.getStore();
    	var records = store.getNewRecords();
    	var records1 = store.getUpdatedRecords();
    	records = records.concat(records1);
    	var data = [];
    	var isok = 1;
    	var err = "";
    	Ext.Array.each(records, function(model){
    		if(model.get('name') == '' || model.get('name') == null){
    			isok = 0;
    			err = "名称不能为空";
    		}
    		data.push(Ext.JSON.encode(model.data));
    	});
    	if(isok == 0){
    		alert("温馨提示:"+err);
    		cellEdit.enable();
    		cellEdit.startEditByPosition({
     			row:0,
     			column:0
     		});
    	}else{
    		if(data.length>0){
   			    Ext.Ajax.request({
    				url: 'province/add.action?province.zdy10='+ escape(data),
    				method: 'GET',
    				timeout: 4000,
    				success: function(response,opts){
    					var result = Ext.JSON.decode(response.responseText);
    					alert("温馨提示:"+result.result);
    					if(result.result == "保存成功"){
    						store.load();
    					}
    				},
    				failure: function(response,opts){
    					var result = Ext.JSON.decode(response.responseText);
    					alert("温馨提示:"+result.result);
    				}
    			});
    		}else{
    			alert("温馨提示:未改动，无需保存");
    		}
    	} 
    },
    
    deletecityClick: function(o, item, rowIndex, colIndex, e){
    	var grid = o.up('areacitylist');
    	var store = grid.getStore();
    	var model = grid.getSelectionModel();
    	if(model.hasSelection()){
	    	var record = model.getLastSelected();
    		var rowIndex = store.indexOf(record);
	    	if(record.get('id') == null || record.get('id') == ''){
	    		store.removeAt(rowIndex);
	    	}else{
	    		var message = confirm("确定要删除："+ record.get('name') +" ？");
	    		if(message == true){
	    			Ext.getBody().mask("数据处理中，请稍等");
  		             var result = "";
  		             Ext.Ajax.request({
  		    				url: 'city/delete.action?cityid='+ record.get('id'),
  		    				method: 'GET',
  		    				timeout: 4000,
  		    				success: function(response,opts){
  		    					var result = Ext.JSON.decode(response.responseText);
  		    					alert("温馨提示:"+result.result);
  		    					if(result.result == "删除成功"){
  		    						store.removeAt(rowIndex);
  		    					}
  	    			    	    Ext.getBody().unmask();
  		    				},
  		    				failure: function(response,opts){
  		    					var result = Ext.JSON.decode(response.responseText);
  		    					Ext.Msg.alert("温馨提示:"+result.result);
  	    			    	    Ext.getBody().unmask();
  		    				}
  		    		 });	
	    		}
	    	}
    	}else{
    		alert("请选择城市");
    	}
    },
	 
	addcityClick: function(o){
		var rec = new M.model.City();
		//var grid = o.ownerCt.ownerCt;
		var grid = o.up('areacitylist');
		var edit = grid.getPlugin('celledit');
		edit.cancelEdit();
		grid.getStore().insert(0, rec);
		edit.startEditByPosition({
			row:0,
			column:1
		});
	 },

    updatecityClick: function(o) {
    	var grid = o.up('areacitylist');
    	cellEdit.completeEdit();
		var store = grid.getStore();
    	var records = store.getNewRecords();
    	var records1 = store.getUpdatedRecords();
    	records = records.concat(records1);
    	var data = [];
    	var isok = 1;
    	var err = "";
		//alert(unescape(getParameter("name")));
    	Ext.Array.each(records, function(model){
    		if(model.get('name') == '' || model.get('name') == null){
    			isok = 0;
    			err = "名称不能为空";
    		}
    		if(model.get('zdy2') == '' || model.get('zdy2') == null){
    			model.set('zdy2', unescape(getParameter("name")));
    		}
    		data.push(Ext.JSON.encode(model.data));
    	});
    	//alert(data);
    	if(isok == 0){
    		alert("温馨提示:"+err);
    		cellEdit.enable();
    		cellEdit.startEditByPosition({
     			row:0,
     			column:0
     		});
    	}else{
    		if(data.length>0){
   			    Ext.Ajax.request({
    				url: 'city/add.action?city.zdy10='+ escape(data),
    				method: 'GET',
    				timeout: 4000,
    				success: function(response,opts){
    					var result = Ext.JSON.decode(response.responseText);
    					alert("温馨提示:"+result.result);
    					if(result.result == "保存成功"){
    						store.load();
    					}
    				},
    				failure: function(response,opts){
    					var result = Ext.JSON.decode(response.responseText);
    					alert("温馨提示:"+result.result);
    				}
    			});
    		}else{
    			alert("温馨提示:未改动，无需保存");
    		}
    	} 
    },
    
    deletetownClick: function(o, item, rowIndex, colIndex, e){
    	store = o.getStore();
    	if(store.getAt(rowIndex).get('id') == null || store.getAt(rowIndex).get('id') == ''){
    		store.removeAt(rowIndex);
    	}else{
    		var message = confirm("确定要删除："+ store.getAt(rowIndex).get('name') +" ？");
    		if(message == true){
    			 Ext.Ajax.request({
	    				url: 'town/delete.action?townid='+ store.getAt(rowIndex).get('id'),
	    				method: 'GET',
	    				timeout: 4000,
	    				success: function(response,opts){
	    					var result = Ext.JSON.decode(response.responseText);
	    					alert("温馨提示:"+result.result);
	    					if(result.result == "删除成功"){
	    						store.removeAt(rowIndex);
	    					}
    			    	    Ext.getBody().unmask();
	    				},
	    				failure: function(response,opts){
	    					var result = Ext.JSON.decode(response.responseText);
	    					alert("温馨提示:"+result.result);
    			    	    Ext.getBody().unmask();
	    				}
	    		 });
    		}
    	}
    },
	 
	addtownClick: function(o){
		var rec = new M.model.City();
		//var grid = o.ownerCt.ownerCt;
		var grid = o.up('areatownlist');
		var edit = grid.getPlugin('celledit');
		edit.cancelEdit();
		grid.getStore().insert(0, rec);
		edit.startEditByPosition({
			row:0,
			column:1
		});
	 },

    updatetownClick: function(o) {
    	var grid = o.up('areatownlist');
    	cellEdit.completeEdit();
		var store = grid.getStore();
    	var records = store.getNewRecords();
    	var records1 = store.getUpdatedRecords();
    	records = records.concat(records1);
    	var data = [];
    	var isok = 1;
    	var err = "";
    	Ext.Array.each(records, function(model){
    		if(model.get('name') == '' || model.get('name') == null){
    			isok = 0;
    			err = "名称不能为空";
    		}
    		if(model.get('zdy2') == '' || model.get('zdy2') == null){
    			isok = 0;
    			err = "所属城市不能为空";
    		}
    		data.push(Ext.JSON.encode(model.data));
    	});
    	//alert(data);
    	if(isok == 0){
    		alert("温馨提示:"+err);
    		cellEdit.enable();
    		cellEdit.startEditByPosition({
     			row:0,
     			column:0
     		});
    	}else{
    		if(data.length>0){
   			    Ext.Ajax.request({
    				url: 'town/add.action?town.zdy10='+ escape(data),
    				method: 'GET',
    				timeout: 4000,
    				success: function(response,opts){
    					var result = Ext.JSON.decode(response.responseText);
    					alert("温馨提示:"+result.result);
    					if(result.result == "保存成功"){
    						store.load();
    					}
    				},
    				failure: function(response,opts){
    					var result = Ext.JSON.decode(response.responseText);
    					alert("温馨提示:"+result.result);
    				}
    			});
    		}else{
    			alert("温馨提示:未改动，无需保存");
    		}
    	} 
    }
});