...
public class ListSupplier {
	...
	public String list(String account,Session session){
		if(account.equalsIgnoreCase("admin")){
			List<Object> o = session.createSQLQuery("select id from supplier where status=1").list();
			if(!o.isEmpty()){
			...
			if(!o0.isEmpty()){
				if(o0.get(0)[0].toString().equalsIgnoreCase("0")){
					List<Object> o = session.createSQLQuery("select id from supplier where status=1 and orgid ="+ o0.get(0)[1].toString()).list();
					if(!o.isEmpty()){
						for(int i=0;i<o.size();i++){
							if(result.equalsIgnoreCase("")){
								result = o.get(i).toString();
					...
					List<Object> o = session.createSQLQuery("select supplierid from users where status=1 and account='"+ account +"'").list();
					if(!o.isEmpty()){
						for(int i=0;i<o.size();i++){
							if(result.equalsIgnoreCase("")){
								result = o.get(i).toString();
