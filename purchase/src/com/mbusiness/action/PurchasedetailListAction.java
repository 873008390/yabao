...
public class PurchasedetailListAction extends ActionSupport implements SessionAware{
	...
	public String list(){
		...
		if(session.get("account") == null || session.get("account").toString() == ""){
			...
			Purchasedetail c = new Purchasedetail();
			c.setId(0);
			c.setZdy3(mmessage.notlogin);
			c.setZdy2("");
			c.setZdy4("");
			c.setZdy1("1");
			purchasedetails.add(c);
			total = 1;
		}else{
			...
			if(!purchasedetails.isEmpty()){				
				total = Integer.parseInt(purchasedetails.get(0).getZdy1());
