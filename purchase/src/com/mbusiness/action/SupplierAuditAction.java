...
public class SupplierAuditAction extends ActionSupport implements SessionAware{
	...
	public String audit(){
		...
		if(session.get("account") == null || session.get("account").toString() == ""){
			...
			result = supplierService.audit(usersession, supplier, idtype);
			if (result.equalsIgnoreCase(mmessage.auditsuccess)){
				success = true;
			}else{
				success = false;
			}
