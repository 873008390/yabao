...
public class AuditgroupdetailListAction extends ActionSupport implements SessionAware{
	...
	public String list(){
		...
		if(session.get("account") == null || session.get("account").toString() == ""){
			...
			Auditgroupdetail c = new Auditgroupdetail();
			c.setId(0);
			c.setZdy3(mmessage.notlogin);
			c.setZdy2("");
			c.setZdy4("");
			c.setZdy1("1");
			auditgroupdetails.add(c);
			total = 1;
		}else{
			usersession.setUsername(session.get("account").toString());
			auditgroupdetails = auditgroupdetailService.list(usersession, idtype, typeid, start, limit, page);
			if(!auditgroupdetails.isEmpty()){				
				total = Integer.parseInt(auditgroupdetails.get(0).getZdy1());
	...
	public void setAuditgroupdetails(List<Auditgroupdetail> auditgroupdetails) {
		this.auditgroupdetails = auditgroupdetails;
	}

	public List<Auditgroupdetail> getAuditgroupdetails() {
