...
public class TownListAction extends ActionSupport implements SessionAware{
	...
	public String list(){
		...
		if(session.get("account") == null || session.get("account").toString() == ""){
			...
			towns = townService.list(usersession, idtype, typeid, start, limit, page);
			if(!towns.isEmpty()){
				total = Integer.parseInt(towns.get(0).getZdy1());
			}else{
				total = 0;
			}
		}
