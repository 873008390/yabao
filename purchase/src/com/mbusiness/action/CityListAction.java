...
public class CityListAction extends ActionSupport implements SessionAware{
	...
	public String list(){
		...
		if(session.get("account") == null || session.get("account").toString() == ""){
			...
			citys = cityService.list(usersession, idtype, typeid, start, limit, page);
			if(!citys.isEmpty()){
				total = Integer.parseInt(citys.get(0).getZdy1());
			}else{
				total = 0;
			}
		}
